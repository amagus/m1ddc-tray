...
- (NSArray<Preset *> *)allPresets {
    NSArray *dicts = [[NSUserDefaults standardUserDefaults] arrayForKey:kPresetsKey];
    if (!dicts) return @[];
...
- (void)applyPreset:(Preset *)preset
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (PresetAction *action in preset.actions) {
            int displayIndex = [controller displayIndexForUUID:action.displayUUID];
            if (displayIndex < 0) {
                allSuccess = NO;
                continue;
            }
            BOOL ok = [controller writeAttribute:action.attributeCode
                                           value:action.value
