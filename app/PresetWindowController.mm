...
@interface PresetWindowController () <NSTableViewDelegate,
...
- (NSView*)buildPresetListPanel {
  ...
  [NSLayoutConstraint activateConstraints:@[
    ...
    [scrollView.bottomAnchor constraintEqualToAnchor:buttonBar.topAnchor
                                            constant:-4],
    [buttonBar.leadingAnchor constraintEqualToAnchor:panel.leadingAnchor
                                            constant:sidePadding],
    [buttonBar.bottomAnchor constraintEqualToAnchor:panel.bottomAnchor
...
- (NSView*)buildPresetEditorPanel {
  ...
  [NSLayoutConstraint activateConstraints:@[
    [_actionsStack.topAnchor constraintEqualToAnchor:actionsDoc.topAnchor
                                            constant:4],
    [_actionsStack.leadingAnchor
        constraintEqualToAnchor:actionsDoc.leadingAnchor],
    [_actionsStack.trailingAnchor
        constraintEqualToAnchor:actionsDoc.trailingAnchor],
    [_actionsStack.bottomAnchor
        constraintEqualToAnchor:actionsDoc.bottomAnchor],
    [_actionsStack.widthAnchor
        constraintEqualToAnchor:_actionsScrollView.widthAnchor
                       constant:-20],
  ]];
  ...
  NSStackView* bottomBar =
      [NSStackView stackViewWithViews:@[ addActionButton, saveButton ]];
  bottomBar.spacing = 8;
  ...
  // Pin everything with explicit constraints — scroll view fills remaining
  // space
  [NSLayoutConstraint activateConstraints:@[
    // Name row
    [nameLabel.topAnchor constraintEqualToAnchor:panel.topAnchor constant:pad],
    [nameLabel.leadingAnchor constraintEqualToAnchor:panel.leadingAnchor
                                            constant:pad],
    [_nameField.centerYAnchor constraintEqualToAnchor:nameLabel.centerYAnchor],
    [_nameField.leadingAnchor constraintEqualToAnchor:nameLabel.trailingAnchor
                                             constant:8],
    [_nameField.trailingAnchor constraintEqualToAnchor:panel.trailingAnchor
                                              constant:-pad],

    // Actions header
    [actionsHeader.topAnchor constraintEqualToAnchor:nameLabel.bottomAnchor
                                            constant:10],
    [actionsHeader.leadingAnchor constraintEqualToAnchor:panel.leadingAnchor
                                                constant:pad],

    // Actions scroll view — fills all remaining space
    [_actionsScrollView.topAnchor
        constraintEqualToAnchor:actionsHeader.bottomAnchor
                       constant:6],
    [_actionsScrollView.leadingAnchor
        constraintEqualToAnchor:panel.leadingAnchor
                       constant:pad],
    [_actionsScrollView.trailingAnchor
        constraintEqualToAnchor:panel.trailingAnchor
                       constant:-pad],
    [_actionsScrollView.bottomAnchor constraintEqualToAnchor:bottomBar.topAnchor
                                                    constant:-10],
...
- (void)populateValuePopup:(NSPopUpButton*)popup
              forAttribute:(UInt8)attrCode
           forDisplayPopup:(NSPopUpButton*)displayPopup
                 withValue:(UInt16)value {
  [popup removeAllItems];

  // Collect options — INPUT uses capabilities cache; others use static arrays
  NSMutableArray<NSDictionary*>* dynOptions = nil;

  if (attrCode == INPUT) {
    NSString* uuid = displayPopup.selectedItem.representedObject;
    int idx = uuid ? [_displayController displayIndexForUUID:uuid] : -1;
    dynOptions =
        idx >= 0
            ? [[_displayController inputOptionsForDisplayIndex:idx] mutableCopy]
            : [NSMutableArray array];
  } else {
    const PresetPopupOption* opts = NULL;
    int cnt = 0;
    switch (attrCode) {
    case PBP:
      opts = kPresetPBPOptions;
      cnt = sizeof(kPresetPBPOptions) / sizeof(kPresetPBPOptions[0]);
      break;
    case PBP_INPUT:
      opts = kPresetPBPInputOptions;
      cnt = sizeof(kPresetPBPInputOptions) / sizeof(kPresetPBPInputOptions[0]);
      break;
    case KVM:
      opts = kPresetKVMOptions;
      cnt = sizeof(kPresetKVMOptions) / sizeof(kPresetKVMOptions[0]);
      break;
    }
    dynOptions = [NSMutableArray array];
    for (int i = 0; i < cnt; i++) {
      [dynOptions addObject:@{
        @"label" : @(opts[i].label),
        @"value" : @(opts[i].value)
      }];
    }
  ...
  BOOL found = NO;
  for (NSDictionary* opt in dynOptions) {
    NSString* title =
        [opt[@"label"] isKindOfClass:[NSString class]]
            ? opt[@"label"]
            : [NSString stringWithUTF8String:[opt[@"label"] UTF8String]];
    UInt16 val = [opt[@"value"] unsignedShortValue];
    [popup addItemWithTitle:title];
    popup.lastItem.tag = val;
    if (val == value) {
  ...
  if (!found && value > 0) {
    ...
    [popup.menu insertItemWithTitle:title
                             action:nil
                      keyEquivalent:@""
                            atIndex:idx];
...
- (void)updateValueControlForRow:(ActionRowView*)row withValue:(UInt16)value {
  ...
  if ([self isPopupAttribute:attrCode]) {
    ...
    [self populateValuePopup:row.valuePopup
                forAttribute:attrCode
             forDisplayPopup:row.displayPopup
                   withValue:value];
  } else {
...
- (ActionRowView*)createActionRow:(PresetAction*)action {
  ...
  // Display popup
  row.displayPopup = [[NSPopUpButton alloc] initWithFrame:NSZeroRect
                                                pullsDown:NO];
  row.displayPopup.translatesAutoresizingMaskIntoConstraints = NO;
  row.displayPopup.target = self;
  row.displayPopup.action = @selector(presetDisplayChanged:);
  for (int i = 0; i < _displayController.displayCount; i++) {
    ...
    row.displayPopup.lastItem.representedObject =
        [_displayController displayUUIDAtIndex:i];
  }
  if (action) {
  ...
  // Attribute popup
  row.attributePopup = [[NSPopUpButton alloc] initWithFrame:NSZeroRect
                                                  pullsDown:NO];
  ...
  for (NSNumber* code in [DisplayController allAttributeCodes]) {
    NSString* name =
        [DisplayController nameForAttribute:code.unsignedCharValue];
  ...
  row.valuePopup = [[NSPopUpButton alloc] initWithFrame:NSZeroRect
                                              pullsDown:NO];
  ...
  // Remove button
  row.removeButton = [NSButton buttonWithTitle:@"✕"
                                        target:self
...
- (void)addPreset:(id)sender {
  ...
  [_presetTable
          selectRowIndexes:[NSIndexSet indexSetWithIndex:_presets.count - 1]
...
- (void)presetDisplayChanged:(NSPopUpButton*)sender {
  // When display changes, refresh the value popup if it's showing an input
  // attribute
  for (ActionRowView* row in _actionRows) {
    if (row.displayPopup == sender) {
      UInt8 attrCode = (UInt8)row.attributePopup.selectedItem.tag;
      if (attrCode == INPUT) {
        [self updateValueControlForRow:row withValue:0];
      }
...
- (void)savePreset:(id)sender {
  (void)sender;
  if (_selectedPresetIndex < 0 ||
      _selectedPresetIndex >= (NSInteger)_presets.count)
    return;
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:@"PresetsDidChange"
                                                      object:nil];
}
