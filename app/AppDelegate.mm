...
- (instancetype)initWithLabel:(NSString *)label width:(CGFloat)width {
    ...
    if (self) {
        ...
        _minusButton = [NSButton buttonWithTitle:@"−" target:self action:@selector(minusTapped:)];
        ...
        _slider = [NSSlider sliderWithValue:50 minValue:0 maxValue:100
                                     target:nil action:nil];
        _slider.translatesAutoresizingMaskIntoConstraints = NO;
        _slider.continuous = YES;

        _plusButton = [NSButton buttonWithTitle:@"+" target:self action:@selector(plusTapped:)];
        ...
        _valueLabel.font = [NSFont monospacedDigitSystemFontOfSize:11 weight:NSFontWeightRegular];
        ...
        [NSLayoutConstraint activateConstraints:@[
            [nameLabel.leadingAnchor constraintEqualToAnchor:self.leadingAnchor constant:20],
            [nameLabel.centerYAnchor constraintEqualToAnchor:self.centerYAnchor],
            [nameLabel.widthAnchor constraintEqualToConstant:70],
            [_minusButton.leadingAnchor constraintEqualToAnchor:nameLabel.trailingAnchor constant:2],
            ...
            [_slider.leadingAnchor constraintEqualToAnchor:_minusButton.trailingAnchor constant:2],
            [_slider.centerYAnchor constraintEqualToAnchor:self.centerYAnchor],
            [_plusButton.leadingAnchor constraintEqualToAnchor:_slider.trailingAnchor constant:2],
            [_plusButton.centerYAnchor constraintEqualToAnchor:self.centerYAnchor],
            [_plusButton.widthAnchor constraintEqualToConstant:20],
            [_valueLabel.leadingAnchor constraintEqualToAnchor:_plusButton.trailingAnchor constant:2],
            ...
            [_spinner.centerXAnchor constraintEqualToAnchor:self.centerXAnchor constant:30],
...
// Input source options for the menu
typedef struct {
    const char *label;
    UInt16 value;
} MenuInputOption;

static const MenuInputOption kMenuInputOptions[] = {
    {"DisplayPort 1", 15}, {"DisplayPort 2", 16},
    {"HDMI 1", 17}, {"HDMI 2", 18}, {"USB-C", 27},
};
static const int kMenuInputOptionCount = sizeof(kMenuInputOptions) / sizeof(kMenuInputOptions[0]);
...
@property (nonatomic, strong) NSMutableDictionary<NSString *, ControlWindowController *> *controlWindows;
...
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSMutableDictionary<NSNumber *, NSNumber *> *> *cachedValues;
@property (nonatomic, strong) NSMutableDictionary<NSString *, NSTimer *> *debounceTimers;
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    _openWindowCount = 0;
    _ddcQueue = dispatch_queue_create("com.m1ddc-tray.menu-ddc", DISPATCH_QUEUE_SERIAL);
    ...
    _statusItem.button.image = [NSImage imageWithSystemSymbolName:@"display"
...
- (void)rebuildMenu {
    ...
    if (_displayController.displayCount == 0) {
        NSMenuItem *noDisplay = [[NSMenuItem alloc] initWithTitle:@"No Displays Found"
        ...
        for (int i = 0; i < _displayController.displayCount; i++) {
            ...
            // Brightness slider
            MenuSliderView *brightnessView = [[MenuSliderView alloc] initWithLabel:@"Brightness" width:menuWidth];
            ...
            // Volume slider
            MenuSliderView *volumeView = [[MenuSliderView alloc] initWithLabel:@"Volume" width:menuWidth];
            ...
            NSMenuItem *inputItem = [[NSMenuItem alloc] initWithTitle:@"Input Source" action:nil keyEquivalent:@""];
            NSMenu *inputMenu = [[NSMenu alloc] init];
            for (int j = 0; j < kMenuInputOptionCount; j++) {
                NSString *title = [NSString stringWithUTF8String:kMenuInputOptions[j].label];
                NSMenuItem *opt = [[NSMenuItem alloc] initWithTitle:title
                                                            action:@selector(switchInput:)
                                                     keyEquivalent:@""];
                opt.tag = kMenuInputOptions[j].value;
                opt.representedObject = uuid;
                opt.target = self;
                [inputMenu addItem:opt];
            }
            // Check current input from cache
            NSNumber *cachedInput = _cachedValues[uuid][@(INPUT)];
            if (cachedInput) {
            ...
            // All Controls...
            NSMenuItem *allControls = [[NSMenuItem alloc] initWithTitle:@"All Controls..."
    ...
    // Presets submenu
    NSMenuItem *presetsItem = [[NSMenuItem alloc] initWithTitle:@"Presets" action:nil keyEquivalent:@""];
    NSMenu *presetsMenu = [[NSMenu alloc] init];
    NSArray<Preset *> *presets = [_presetManager allPresets];

    if (presets.count == 0) {
        NSMenuItem *noPresets = [[NSMenuItem alloc] initWithTitle:@"No Presets" action:nil keyEquivalent:@""];
    ...
    NSMenuItem *editPresets = [[NSMenuItem alloc] initWithTitle:@"Edit Presets..."
    ...
    // Refresh
    NSMenuItem *refresh = [[NSMenuItem alloc] initWithTitle:@"Refresh Displays"
...
- (void)openControlWindow:(NSMenuItem *)sender {
    ...
    if (!wc) {
        wc = [[ControlWindowController alloc] initWithDisplayIndex:index
...
- (void)applyPreset:(NSMenuItem *)sender {
    ...
    [_presetManager applyPreset:preset
        ...
        if (!success) {
            ...
            alert.informativeText = @"Some actions could not be applied. The display may be disconnected or the attribute unsupported.";
...
- (void)menuSliderChanged:(NSSlider *)sender {
    ...
    // Update cache
    if (!_cachedValues[uuid]) _cachedValues[uuid] = [NSMutableDictionary dictionary];
    ...
    NSTimer *timer = [NSTimer timerWithTimeInterval:0.1
        repeats:NO block:^(NSTimer *t __unused) {
...
- (void)switchInput:(NSMenuItem *)sender {
    ...
    // Update cache
    if (!_cachedValues[uuid]) _cachedValues[uuid] = [NSMutableDictionary dictionary];
    _cachedValues[uuid][@(INPUT)] = @(inputValue);

    dispatch_async(_ddcQueue, ^{
        [self->_displayController writeAttribute:INPUT
                                           value:inputValue
                                 forDisplayIndex:displayIndex];
    });
}
...
- (void)menuWillOpen:(NSMenu *)menu {
    ...
    dispatch_async(_ddcQueue, ^{
        ...
        DDCValue volume = [self->_displayController readAttribute:VOLUME
                                                 forDisplayIndex:displayIndex];
        DDCValue input = [self->_displayController readAttribute:INPUT
                                                forDisplayIndex:displayIndex];
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (NSMenuItem *item in menu.itemArray) {
                if (![item.view isKindOfClass:[MenuSliderView class]]) continue;
                ...
                if (sv.attrCode == LUMINANCE && brightness.curValue >= 0) {
                    ...
                    sv.valueLabel.stringValue = [NSString stringWithFormat:@"%d", brightness.curValue];
                    ...
                    sv.valueLabel.stringValue = [NSString stringWithFormat:@"%d", volume.curValue];
            ...
            if (input.curValue >= 0) {
                self->_cachedValues[uuid][@(INPUT)] = @(input.curValue);
                for (NSMenuItem *item in menu.itemArray) {
                    if (!item.submenu) continue;
                    for (NSMenuItem *opt in item.submenu.itemArray) {
                        opt.state = (opt.tag == input.curValue)
...
- (void)quitApp:(id)sender {
    (void)sender;
    // Force quit — close all windows first so applicationShouldTerminate allows it
...
- (void)windowDidClose:(NSNotification *)notification {
    ...
    _openWindowCount = (int)_controlWindows.count + (_presetWindowController ? 1 : 0);
    [self updateActivationPolicy];
}
